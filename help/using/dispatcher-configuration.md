...
| `[ and ]` | 取消標記字元類別的開頭和結尾。 字元類別可包含一或多個字元範圍及單一字元。<br/>如果目標字元符合字元類別中或定義的範圍內的任何字元，就會有符合專案。<br/>如果未包含右括弧，則模式不會產生符合專案。 | `*[o]men.html*`<br/> 符合下列 HTTP 請求：<br/><ul><li>`"GET /content/geometrixx-outdoors/en/women.html"`</li></ul><br/>不符合下列 HTTP 請求：<br/><ul><li>`"GET /content/geometrixx-outdoors/en/men.html"`</li></ul><br/> `*[o/]men.html*` <br/>符合下列 HTTP 請求：<br/><ul><li>`"GET /content/geometrixx-outdoors/en/women.html"`</li><li>`"GET /content/geometrixx-outdoors/en/men.html"`</li></ul> |
...
工作階段管理員 (設定包含 `sessionmanagement` 節點) 控管陣列的快取，而且使用者的工作階段無效或不再有效。
