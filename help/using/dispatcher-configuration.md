...
根據預設，Dispatcher 設定會儲存在 `dispatcher.any` 文字檔案中，但是您在安裝期間可以變更此檔案的名稱和位置。
...
例如，如果`dispatcher.any`檔案與快取目錄位於相同的目錄中，則可以針對[docroot](#specifying-the-cache-directory)屬性使用下列值：
...
## 為 Dispatcher 實例命名 {#naming-the-dispatcher-instance-name}
...
`/clientheaders` 屬性會定義 Dispatcher 從用戶端 HTTP 要求傳送至轉譯器 (AEM 實例) 的 HTTP 標頭清單。
...
### 虛擬主機解析範例 {#example-virtual-host-resolution}
...
>[!CAUTION]
>
>在 `/cache` 區段將 `/allowAuthorized` 設定為 `"0"`，以啟用此功能。 如[「使用驗證時快取」](#caching-when-authentication-is-used)區段所述，當您設定 `/allowAuthorized 0 ` 時，包括驗證資訊的請求&#x200B;**不會**&#x200B;進行快取。 如需進行權限敏感型快取，請參閱[快取安全內容](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-dispatcher/using/configuring/permissions-cache)頁面。
...
#### HTTP 要求的請求行部分 {#the-request-line-part-of-http-requests}
...
#### 雙引號與單引號的使用 {#double-quotes-vs-single-quotes}
...
在 Dispatcher 4.2.0 之後的版本中，您可以在篩選器模式中包括 POSIX 擴充型規則運算式。

#### 篩選器疑難排解 {#troubleshooting-filters}

如果篩選器的觸發方式與您的預期不符，請啟用 Dispatcher 的[追蹤記錄](#trace-logging)，以便查看哪個篩選器正在攔截請求。

#### 篩選器範例：全部拒絕 {#example-filter-deny-all}
...
對明確拒絕的區域發出請求會傳回 404 錯誤碼 (找不到頁面)。

#### 篩選器範例：拒絕存取特定區域 {#example-filter-deny-access-to-specific-areas}

篩選器也可讓您拒絕存取各種元素 (例如 ASP 頁面) 以及發佈執行個體中的敏感區域。 以下篩選器拒絕存取 ASP 頁面：

```xml
/0002  { /type "deny" /url "*.asp"  }
```

#### 篩選器範例：啟用 POST 要求 {#example-filter-enable-post-requests}

以下篩選器範例允許透過 POST 方法提交表單資料：
...
#### 篩選器範例：允許存取 Workflow 控制台 {#example-filter-allow-access-to-the-workflow-console}
...
>[!NOTE]
>
>將多個篩選器模式套用至一個請求時，最後一個套用的篩選器模式會生效。

#### 篩選器範例：使用規則運算式 {#example-filter-using-regular-expressions}

此篩選器使用以單引號來定義的規則運算式，在非公開內容目錄中啟用副檔名：

```xml
/005  {  /type "allow" /extension '(css|gif|ico|js|png|swf|jpe?g)' }
```

#### 篩選器範例：篩選請求 URL 的額外元素 {#example-filter-filter-additional-elements-of-a-request-url}
...
當您設定 Dispatcher 時，要盡可能限制外部存取。 以下範例對外部訪客提供最低限度的存取權：

* `/content`
* 各種其他內容，如設計和用戶端程式庫等。 例如：
...
>[!CAUTION]
>
>如果您[在發佈環境中使用報告](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-65/content/sites/administering/operations/reporting#using-reports-in-a-publish-environment)，應該將 Dispatcher 設定為拒絕外部訪客存取 `/etc/reports`。
...
>
>在上述範例中，如果也要允許對 `/etc` 發出且沒有查詢字串的請求，則需要以下規則：
>
...
若要讓 Dispatcher 快取失效，並確定您收到代碼 403 回應，請在終端機或命令提示字元中發出以下命令：

`curl -H "CQ-Handle: /content" -H "CQ-Path: /content" https://yourhostname/dispatcher/invalidate.cache`

## 允許存取虛名 URL {#enabling-access-to-vanity-urls-vanity-urls}
...
Dispatcher **版本 4.3.6** 已經加入新的 `/loadOnStartup` 參數。 您可以使用這個參數來設定在啟動時載入虛名 URL，如下所示：

透過加入 `/loadOnStartup 0` (請參閱以下範例)，您可以停用在啟動時載入虛名 URL。
...
雖然 `/loadOnStartup 1` 會在啟動時載入虛名 URL，請記住 `/loadOnStartup 1` 是此參數目前的預設值。

## 轉寄整合請求 - `/propagateSyndPost` {#forwarding-syndication-requests-propagatesyndpost}
...
`/serveStaleOnError` 屬性可控制當轉譯伺服器傳回錯誤時，Dispatcher 是否傳回失效的文件。 根據預設，在接觸了 statfile 並讓快取內容失效時，Dispatcher 在下次請求該內容時會刪除快取的內容。 此動作將在下次請求時執行。
...
* 從`/content/wknd/us`資料夾中移除所有模式為en.*的檔案。
...
>[!NOTE]
>
>設定glob屬性時，它應符合查詢引數名稱。例如，如果您想忽略下列URL `http://example.com/path/test.html?p1=test&p2=v2`中的&quot;p1&quot;引數，則glob屬性應為：
...
`mode` 屬性會指定哪些檔案權限會套用至快取中的新目錄和檔案。 呼叫程序的 `umask` 會限制此設定。 這是一個八進位數字，由下列一個或多個值的總和所構成：
...
### 限制 .stat 檔案的觸踫 {#throttling-stat-file-touching}
...
### 設定時間型快取失效 - `/enableTTL` {#configuring-time-based-cache-invalidation-enablettl}
...
### 在 Dispatcher 統計資料中反映伺服器不可用的情況 {#reflecting-server-unavailability-in-dispatcher-statistics}
...
## 指定黏性連線資料夾 - `/stickyConnectionsFor` {#identifying-a-sticky-connection-folder-stickyconnectionsfor}
...
使用 `/health_check` 屬性指定在出現 500 狀態代碼時要檢查的 URL。 如果此頁面也傳回 500 狀態代碼，則會將執行個體視為無法使用，並在重試之前將可設定的時間懲罰 ( `/unavailablePenalty`) 套用到轉譯器。
...
`/retryDelay` 屬性會設定 Dispatcher 每次嘗試與陣列轉譯器建立連線之間等待的時間 (以秒為單位)。 針對每一輪，Dispatcher 嘗試與轉譯器建立連線的次數上限為陣列中的轉譯器數目。
...
如果系統呼叫的物件位在透過 NFS 的方式存取的遠端系統上，則任何檔案系統導向的系統呼叫都可以被中斷 `EINTR`。 這些系統呼叫是否可以逾時或中斷，取決於基礎檔案系統採用何種方式裝載在本機電腦上。

如果您的實例具有這類設定，且記錄中包含以下訊息，請使用 `/ignoreEINTR` 參數：
...
這類訊息會在下列情況中產生：當 `read more data` 區段中出現 `EINTR`， 發生此問題是因為訊號比所有資料更早抵達。
...
## 設計 Glob 屬性的模式 {#designing-patterns-for-glob-properties}
...
| `[ and ]` | 取消標記字元類別的開頭和結尾。 字元類別可以包含一或多個字元範圍及單一字元。<br/>如果目標字元符合字元類別中或定義範圍內的任何字元，就會有符合專案。<br/>如果未包含右中括號，則模式不會產生任何符合項目。 | `*[o]men.html*`<br/> 符合下列 HTTP 請求：<br/><ul><li>`"GET /content/geometrixx-outdoors/en/women.html"`</li></ul><br/>不符合下列 HTTP 請求：<br/><ul><li>`"GET /content/geometrixx-outdoors/en/men.html"`</li></ul><br/> `*[o/]men.html*` <br/>符合下列 HTTP 請求：<br/><ul><li>`"GET /content/geometrixx-outdoors/en/women.html"`</li><li>`"GET /content/geometrixx-outdoors/en/men.html"`</li></ul> |
...
陣列的組態不包含主目錄（組態元素`cache.docroot`）。
...
* **not cacheable： request need be a GET or HEAD**
...
* **not cacheable: session manager needs to authenticate**\
  工作階段管理員 (設定包含 `sessionmanagement` 節點) 控管陣列的快取，而且請求未包含適當的驗證資訊。
...
工作階段管理員（設定包含`sessionmanagement`節點）會控制陣列的快取，而且使用者的工作階段無效或不再有效。
* **not cacheable： response contain`no_cache`**
遠端伺服器傳回`Dispatcher: no_cache`標頭，禁止Dispatcher快取輸出。
* **not cacheable： response內容長度為零**
回應的內容長度為零；Dispatcher不會建立長度為零的檔案。
