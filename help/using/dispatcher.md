...
# Dispatcher 概觀 {#dispatcher-overview}
...
>
>但是，Dispatcher 也可用來提高您&#x200B;**編寫執行個體**&#x200B;的回應速度。這是事實，尤其是如果您有大量使用者編輯和更新您的網站時特別實用。如需此情況特有的詳細資訊，請參閱下方的[搭配製作伺服器使用 Dispatcher](#using-a-dispatcher-with-an-author-server)。

## 為何要使用 Dispatcher 實施快取？ {#why-use-dispatcher-to-implement-caching}
...
>
>檔案名稱的長度可能有作業系統層級的限制。也就是說，如果您的 URL 包含多個選擇器，可能會遇到這個問題。
...
Dispatcher 會將快取的檔案儲存在網頁伺服器上，如同靜態網站的一部分。如果使用者請求可快取文件，Dispatcher 會將檢查該文件是否存在於 Web 伺服器的檔案系統中：
...
有關 AEM 每個實例處理文件的速度，Dispatcher 會保留內部統計資料。Dispatcher 會根據此資料，估計在回應請求時哪個執行個體可提供最快的回應時間，藉此來保留該執行個體所需的運算時間。
...
## 搭配內容傳遞網路使用 Dispatcher {#using-dispatcher-with-a-cdn}
...
## 控制內容傳遞網路快取 {#controlling-a-cdn-cache}
...
>[!NOTE]
>
>另請參閱 [AEM (CQ) Dispatcher 安全性和內容傳遞網路 + 瀏覽器快取](https://www.slideshare.net/andrewmkhoury/dispatcher-caching-aemgemspart2jan2015)，以及 [Dispatcher 快取](https://experienceleague.adobe.com/zh-hant/docs/events/experience-manager-gems-recordings/gems2015/aem-dispatcher-caching-new-features-and-optimizations)的相關錄製簡報。

## 搭配製作伺服器使用 Dispatcher {#using-a-dispatcher-with-an-author-server}

>[!CAUTION]
>
>如果您使用[具有 Touch UI 的 AEM](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-65/content/implementing/developing/introduction/touch-ui-concepts)，請&#x200B;**不要**&#x200B;快取作者實例內容。如果作者實例已啟用快取，則必須停用並刪除快取目錄的內容。如要停用快取，請編輯 `author_dispatcher.any` 檔案，並修改 `/rule` 屬性 (`/cache` 區段)，如下所示：
