---
title: Dispatcher 總覽
description: 瞭解如何使用Adobe Experience Manager Dispatcher來改善AEMCloud Service的安全性、快取行為等。
...
>
>但是，Dispatcher也可用來提高的回應速度 **作者執行個體**. 這倒是事實，特別是如果您有大量使用者編輯和更新您的網站。 如需此情況特有的詳細資訊，請參閱下方的[搭配撰寫伺服器使用 Dispatcher](#using-a-dispatcher-with-an-author-server)。
...
此過程非常簡單且有效率。 如果訪客要求檔案 (例如 HTML 頁面)，則通常會直接從記憶體擷取檔案，不然頂多是從本機磁碟讀取檔案。 靜態Web伺服器已上市相當長一段時間。 因此，有各種管理和安全管理工具。 這些工具與網路基礎架構高度整合。
...
**快取目錄**&#x200B;針對快取，Dispatcher 模組會使用網頁伺服器的功能提供靜態內容。Dispatcher會將快取的檔案置於網頁伺服器的根目錄中。
...
>
>檔案名稱的長度可能受到作業系統層級的限制。 也就是說，如果您有一個包含多個選擇器的 URL。
...
1. 它會從快取中刪除修改過的檔案。
1. 它會從快取中刪除以相同名稱開頭的所有檔案。例如，如果檔案 `/en/index.html` 更新，開頭為 `/en/index.` 都會被刪除。 此機制可讓您設計快取效率高的網站，尤其是圖片導覽的網站。
...
您可以[定義 Dispatcher 會在設定檔案中快取哪些文件](https://experienceleague.adobe.com/en/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration)。 Dispatcher 會根據可快取文件清單來檢查請求。如果文件不在此清單中，Dispatcher 會請求 AEM 執行個體的文件。

在下列情況下，Dispatcher 一律會直接從 AEM 執行個體要求文件：

* 請求URI包含問號 `?`. 這種情況通常表示這是一個不需要快取的動態頁面，例如搜尋結果。
* 缺少副檔名。 網頁伺服器需要副檔名來判斷文件類型 (MIME 類型)。
* 驗證標頭已設定 (可設定)。

>[!NOTE]
>
>Dispatcher 可快取 GET 或 HEAD (用於 HTTP 標頭) 方法。如需有關回應標頭快取的其他資訊，請參閱[快取 HTTP 回應標頭](https://experienceleague.adobe.com/en/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration)一節。
...
>[!CAUTION]
>
>雖然單一Dispatcher就能夠使可用Publish執行個體的容量飽和，但對於某些罕見的應用程式而言，在兩個Dispatcher執行個體之間平衡負載也是合理的。 必須仔細考量含有多個Dispatcher的設定。 原因是額外的Dispatcher可能會增加可用Publish執行個體的負載，且可能輕鬆降低大多數應用程式中的效能。
...
由於黏著連線會限制 Dispatcher 最佳化請求的功能，因此您應該視需要來使用。您可以指定包含「粘著」檔案的資料夾，如此可確保該資料夾中的所有檔案都是針對每位使用者在相同例項中撰寫。
...
CDN是HTTP基礎架構元件，其運作方式與Dispatcher類似。 CDN節點收到要求時，會儘可能從快取中提供要求（該資源可在快取中取得且有效）。 否則，會前往下一個最近的伺服器，為進一步的請求擷取資源並加以快取 (如適用)。
...
在一般的AEM設定中，依副檔名、路徑或兩者進行設定（可透過上述第1和第2點達成），可讓您設定合理的快取期間。 這些快取期間適用於不會經常變更的常用資源，例如設計影像和使用者端程式庫。 部署新版本時，通常需要手動失效。

如果將此方法用於快取受管理的內容，則表示只有在已設定的快取期限過期後，使用者才能看到內容變更。 而且，當再次從Dispatcher擷取檔案時。
...
>[!NOTE]
>
>另請參閱 [AEM (CQ) Dispatcher 安全性和 CDN+瀏覽器快取](https://www.slideshare.net/andrewmkhoury/dispatcher-caching-aemgemspart2jan2015)，以及關於 [Dispatcher 快取](https://experienceleague.adobe.com/en/docs/events/experience-manager-gems-recordings/gems2015/aem-dispatcher-caching-new-features-and-optimizations)的相關錄製簡報。

## 搭配撰寫伺服器使用 Dispatcher {#using-a-dispatcher-with-an-author-server}

>[!CAUTION]
>
>如果您使用[具有 Touch UI 的 AEM](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/developing/introduction/touch-ui-concepts)，請&#x200B;**不要**&#x200B;快取編寫執行個體內容。 如果已為編寫執行個體啟用快取，則必須停用並刪除快取目錄的內容。 如要停用快取，請編輯 `author_dispatcher.any` 檔案，並修改 `/rule` 屬性 (`/cache` 區段)，如下所示：
...
1. 確保Dispatcher能夠透過TCP/IP連線到您的編寫執行個體。
...
1. 在文字編輯器中開啟 `author_dispatcher.any`，並進行下列變更: 
   ...
   1. 變更 `/docroot` (`/cache` 區段)，將它們指向快取目錄。 如果您正在使用[具有 Touch UI 的 AEM](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/developing/introduction/touch-ui-concepts)，請參閱上面的警告。
   1. 儲存變更。

1. 刪除所有現有檔案 `/cache` > `/docroot` 您在上面設定的目錄。
1. 重新啟動網頁伺服器。

>[!NOTE]
>
>搭配提供的 `author_dispatcher.any` 設定，當您安裝CQ5 Feature Pack、Hotfix或應用程式程式碼套件時，此設定會影響下的任何內容 `/libs` 或 `/apps`，您必須刪除快取的檔案。 這些檔案位於Dispatcher快取中的這些目錄下。 這麼做會確保下次請求時會擷取新升級的檔案，而非舊的快取檔案。
