---
title: 快取安全內容
description: 了解 Dispatcher 如何進行權限敏感型快取。
contentOwner: User
...
## 實施權限敏感型快取 {#implementing-permission-sensitive-caching}
...
>[!NOTE]
>
>當Dispatcher前面有CDN （或任何其他快取）時，您應該相應地設定快取標題，讓CDN不會快取私人內容。例如： `Header always set Cache-Control private`。
>若為AEM as a Cloud Service，請參閱[快取](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-cloud-service/content/implementing/content-delivery/caching)頁面，以取得有關如何設定私人快取標頭的詳細資訊。

## 建立 Auth Checker servlet {#create-the-auth-checker-servlet}

建立並部署一個 servlet 來針對請求網頁內容的使用者執行驗證和授權工作。 此 servlet 可使用任何驗證方法。 也可以使用任何授權方法。 例如，可以使用 AEM 使用者帳戶和存放庫 ACL。 或者，可以使用 LDAP 查詢服務。 您將此 servlet 部署到 Dispatcher 當作轉譯器使用的 AEM 執行個體。
...
>[!NOTE]
>
>在 `Sling` Servlet Resolver (org.apache.sling.servlets.resolver.SlingServletResolver) 服務中必須啟用 sling.servlet.paths 屬性的值。
