...
Dispatcher提供數個內建機制，您可用來將效能最佳化。 本節將說明如何設計網站，好讓快取的優點最大化。

>[!NOTE]
>
>記住 Dispatcher 會將快取儲存在標準網頁伺服器上可能會有所幫助。 這表示您：
>
>* 可快取可以儲存為頁面並使用URL請求的所有內容
>* 無法儲存其他資料，例如HTTP標頭、Cookie、工作階段資料和表單資料。
>
>一般而言，許多快取策略與選取良好URL且不依賴這些額外資料有關。

## 使用一致的頁面編碼 {#using-consistent-page-encoding}

不會快取HTTP要求標頭，因此如果您將頁面編碼資訊儲存在標頭中，可能會發生問題。 在此情況下，當 Dispatcher 從快取中提供頁面時，將會對此頁面使用網頁伺服器的預設編碼。 有兩種方式可避免此問題：
...
>[!NOTE]
>
>對於大多數版面方面，也可以使用樣式表及/或使用者端指令碼。 這些通常可以與快取搭配使用。
>
>這對於列印版本也很有用，您可以在其中使用URL，例如：
>
...
如果您將頁面標題或其他文字轉譯為圖片，請儲存檔案，以便在頁面內容更新時將其刪除：
...
如果您將圖片用於導覽專案，方法基本上與標題相同，只是稍微複雜一點。 將所有導覽影像與目標頁面一起儲存。 如果您將兩張圖片用於一般和活躍情境，可以使用以下指令碼：
...
[黏性連線](dispatcher.md#TheBenefitsofLoadBalancing)可確保同一個使用者的文件都會在相同伺服器上編寫。 如果使用者離開此資料夾並於稍後返回，連線仍然保持不變。 定義一個資料夾，使其可容納所有需要網站的粘性連線的檔案。 試著不要將其他文件放在該資料夾中。 如果您使用個人化頁面和工作階段資料，這會影響負載平衡。

## MIME 類型 {#mime-types}

瀏覽器可使用兩種方式來判斷檔案的類型：

1. 透過其副檔名（例如.html、.gif和.jpg）
...
* 確定檔案總是有適當的副檔名。
* 避免使用具有像是 download.jsp?file=2214 等 URL 的通用檔案服務指令碼。 重寫指令碼，使其使用包含檔案規格的URL。 在上一個範例中，這將會是 `download.2214.pdf`.
