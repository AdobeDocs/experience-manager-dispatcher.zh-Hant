...
例如，一家公司為其兩個品牌發佈網站：品牌A和品牌B。網站頁面的內容會在AEM中撰寫，並儲存在相同的存放庫工作區中：
...
使用Dispatcher快取內容時，在使用者端HTTP請求中的頁面URL、對應快取檔案的路徑與存放庫中對應檔案的路徑之間必須建立關聯。
...
1. 如果快取檔案無效，Dispatcher會向AEM發佈執行個體請求最新轉譯的頁面。
...
* a - AEM編寫執行個體上會啟用一個頁面，並將內容復寫到發佈執行個體。
* b - Dispatcher Flush 代理程式會呼叫 Dispatcher，好讓複寫內容的快取失效。
* c - Dispatcher 會觸及一個或多個 .stat 檔案，好讓快取檔案失效。

若要在多個網域中使用Dispatcher，您必須設定AEM、Dispatcher和您的網頁伺服器。 此頁面上所述的解決方案是通用的，適用於大多數環境。 由於某些 AEM 拓撲很複雜，您的解決方案可能需要進一步的自訂設定才能解決特定問題。 您可能需要改寫範例，以符合您現有的IT基礎結構和管理政策。
...
建議您針對網頁使用簡短URL。 通常頁面 URL 會反映包含網頁內容的存放庫資料夾的結構。 不過，URL 不會揭露最上層的存放庫節點，例如 `/content`。 用戶端不一定會察覺 AEM 存放庫的結構。
...
* 在Linux®系統上部署AEM製作和發佈執行個體。
* Apache HTTPD是部署在Linux®系統上的Web伺服器。
...
使用此設定時，網頁伺服器在收到以下請求時會執行以下操作 `https://branda.com/en/products.html`：
...
虛擬主機會繼承 [DispatcherConfig](dispatcher-install.md#main-pars-67-table-7) 在主要伺服器區段中設定的屬性值。 虛擬主機可包含其自身的 DispatcherConfig 屬性以覆寫主要伺服器設定。
...
此外，AEM發佈執行個體必須指定為虛擬主機的轉譯器。 視需要設定其他陣列屬性。 以下程式碼是 branda.com 網域的簡化陣列設定：
...
* `/docroot` 屬性會設定為網頁伺服器的預設 docroot。 通常這是 `/content` 資料夾建立所在的目錄。 Linux®上的Apache的範例值為 `/usr/lib/apache/httpd-2.4.3/htdocs`.
...
建立內容頁面的對應之後，若要探索更多必要的對應，請使用網頁瀏覽器在網頁伺服器上開啟頁面。 在發佈執行個體的 error.log 檔案中，找出有關找不到資源的訊息。 以下範例訊息指示需要 `/etc/clientlibs` 的對應：
...
>[!NOTE]
>
>預設Apache Sling重寫程式的Linkchecker轉換器會自動修改頁面中的超連結，以避免連結失效。 不過，只有當連結目標是 HTML 或 HTM 檔案時，才會執行連結重寫。 若要更新其他檔案類型的連結，請建立轉換器元件，並將其新增到 HTML 重寫程式管道。
...
若要將網域型URL轉譯為Dispatcher快取中的檔案路徑，請使用網頁伺服器的內部URL重寫功能。 例如，對 `https://brandA.com/en.html` 頁面的用戶端請求會轉譯成網頁伺服器的主目錄中的 `content/sitea/en.html` 檔案。
...
當資源路徑無法在網頁伺服器上下文中正確解析時，請重寫參照。 例如，當產生影像的元件建立類似 /content/sitea/en/products.navimage.png 等連結時，需要使用轉換器。 此 `topnav` 的元件 [如何建立完整功能的網際網路網站](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/developing/introduction/the-basics) 會建立這類連結。
...
* **轉換器：** 偵聽SAX事件，因此修改事件目標(HTML元素)。 重寫程式管道包含零個或多個轉換器。 轉換器會依序執行，將 SAX 事件傳遞給序列中的下一個轉換器。
...
1. 實作 `org.apache.sling.rewriter.Transformer` 介面。 若要將工作減到最少，您可以擴充 `org.apache.cocoon.xml.sax.AbstractSAXPipe` 類別。 若要自訂重寫行為，請覆寫startElement方法。 傳遞給轉換器的每個 SAX 事件都會呼叫此方法。
1. 組合並部署類別。
1. 若要將轉換器新增到管道，請新增設定節點到您的AEM應用程式。

>[!TIP]
>您可以改為將 TransformerFactory 設定為將轉換器插入到每個定義的重寫程式中。 因此，您不需要設定管道：
>
...
>[!NOTE]
>
>若要建立您的Maven專案，請使用 [多模組](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) 內容套件Maven外掛程式的原型。 POM 會自動建立及安裝內容套件。

以下範例實作的轉換器會重寫影像檔案的參照。

* MyRewriterTransformerFactory 類別會將 MyRewriterTransformer 物件具現化。 pipeline.type屬性會將轉換器別名設為 `mytransformer`. 若要在管道中包含別名，管道設定節點應在轉換器清單中包含別名。
* MyRewriterTransformer 類別會覆寫 AbstractSAXTransformer 類別的 startElement 方法。 startElement 方法會針對 img 元素重寫 src 屬性的值。

這些範例並不健全。 請勿在生產環境中使用它們。
