...
Dispatcher 是 Adobe Experience Manager 的快取工具或負載平衡工具，或兩者皆是，可協助實現快速且動態的網頁撰寫環境。 對於快取，Dispatcher 會當作 HTTP 伺服器 (例如 Apache) 的一部分來運作。 它的目的是儲存 (或「快取」) 盡可能多的靜態網站內容。 並盡量少存取網站引擎的版面配置。 在負載平衡角色中，Dispatcher 會將使用者請求 (負載) 分散到不同的 AEM 執行個體 (轉譯器)。

對於快取，Dispatcher 模組會使用網頁伺服器的功能提供靜態內容。 Dispatcher 會將快取的文件置於網頁伺服器的文件根目錄。

### Dispatcher 如何執行快取？

Dispatcher 會使用網頁伺服器的功能提供靜態內容。 Dispatcher 會將快取的文件存放於網頁伺服器的文件根目錄。 Dispatcher 有兩種主要方法，用於在對網站進行更改時更新快取內容。
...
由於黏性連線會限制 Dispatcher 最佳化請求的能力，因此您應該視需要使用此做法。 您可以指定包含「黏性」文件的資料夾，藉此確保該資料夾中的所有文件都會在使用者的相同執行個體中處理。
...
en目錄底下名為`_jcr_content`的任何目錄（如果存在的話，會包含頁面的子節點的快取呈現）。
只有在`CQ-Action`是`Delete`或`Deactivate`時，才會刪除目錄`en`。
...
請參閱 [Dispatcher 安全性檢查清單](security-checklist.md)和 [AEM 安全性檢查清單](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-64/administering/security/security-checklist)頁面。

### Dispatcher 問題：`jcr:content` 已變成 `jcr%3acontent`

**問**：企業最近在 Dispatcher 層級遇到了問題。 從 CQ 存放庫獲取一些資料的 AJAX 呼叫之一包含 `jcr:content`。 而那個被編碼至 `jcr%3acontent` 並導致錯誤的結果集。

**答案**：呼叫 `ResourceResolver.map()` 以產生易記的 GET 要求 URL，並解決 Dispatcher 快取問題。 map() 方法將 `:` 冒號編碼為下劃線，而 resolve() 方法將它們重新解碼為 SLING JCR 可讀格式。 使用 map() 方法產生在 Ajax 呼叫中使用的 URL。
...
請參閱[複寫](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-65/content/implementing/deploying/configuring/replication#configuring-your-replication-agents)頁面。
...
1. 前往每個代理程式的設定。
1. 在「**觸發器**」標籤上勾選「**接收時**」方塊。

## 其他

Dispatcher如何判斷檔案是否為最新版本？
為了判斷檔案是否為最新版本，Dispatcher會執行下列動作：

它會檢查檔案是否會自動失效。如果不會，則將該檔案視為最新版本。
如果檔案已設定為自動失效，Dispatcher會檢查其是否比最後一次可用變更來得舊或更新。如果版本較舊，Dispatcher會向AEM執行個體要求最新版本，並取代快取中的版本。
